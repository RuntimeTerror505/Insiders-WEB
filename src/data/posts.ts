export type Post = {
  ...
  category: string;
  title: string;
  /** Лід під заголовком. Показується у великих постах угорі сторінки */
  excerpt?: string;
  ...
  /** Два таких пости стоять окремим блоком над сіткою */
  featured?: boolean;
};
...
export const gridPosts = posts.filter((post) => !post.featured);
